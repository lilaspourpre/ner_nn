    ...
    if len(list_of_tags) == len(list_of_tokens):
        ...
        for index in range(len(list_of_tokens)):
            if not ((ne_tag is not None) ^ (ne_words != [])):
                ...
                if current_tag.startswith('B') or current_tag.startswith('I') or current_tag.startswith('L'):
                    dict_of_final_ne, ne_words, ne_tag = __check_bil(dict_of_final_ne, ne_words, ne_tag, current_tag,
                                                                     current_token)
                elif current_tag.startswith('O'):
                    dict_of_final_ne, ne_words, ne_tag = __check_o(dict_of_final_ne, ne_words, ne_tag)
        ...
        if ne_words != [] and ne_tag is not None:
