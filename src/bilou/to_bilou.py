...
# XXX eclipse shows reasonable warnings: unused variable and assignment to reserved build-in symbols

def get_tagged_tokens_from(dict_of_nes, token_tuple): # XXX why token_tuple
    """
    ...
    """
    list_of_tagged_tokens = ['O' for i in range(len(token_tuple))] # XXX mixture of types

    for ne in dict_of_nes.values():
        for id in ne['tokens_list']:
            tag = format_tag(id, ne)
            id_in_token_tuple = [token.get_id() for token in token_tuple].index(id) # XXX trash!!!
            token = token_tuple[id_in_token_tuple]
            list_of_tagged_tokens[id_in_token_tuple] = TaggedToken(tag, token)
    # XXX mixture of types returned
